...
proven bottleneck. All numbers below are from \code{doc/optimizations.md} on the
same 32-split workload.
...
\subsection{Optimisations that do \emph{not} move the wall}

Neither optimisation changes the Amdahl serial fraction much: the
curve keeps its shape, just lower in absolute time (see \S\ref{sec:amdahl}).
Future ideas (logged but not all benchmarked): gzip partitions before transfer,
\emph{push} partitions after MAP instead of pulling, replace SSH with raw TCP,
and a C++ worker (a prototype on the \code{cpp-worker} branch gives $\sim$1.7$\times$
compute).
...
shared machine it hits a wall near $3\times$ ($f=0.349$). Optimisations
(\S\ref{sec:opt}) lower absolute time but barely move $f$, textbook Amdahl
behaviour, demonstrated empirically and reproducibly.
...
\paragraph{Where the current code is still wasteful.} The reduce loop
first does \code{readlines()} per source and accumulates \emph{every} incoming
line into one list before folding it into the counter. That is fine at a few MB,
but it is $O(\text{all incoming lines})$ and \emph{would} saturate at TB scale.
